...
export function array(lexicons, path, def, value) {
    // type
    ...
    if (typeof def.maxLength === 'number') {
        if (value.length > def.maxLength) {
            return {
                success: false,
                error: new ValidationError(`${path} must not have more than ${def.maxLength} elements`),
            };
        }
    }
    // minLength
    if (typeof def.minLength === 'number') {
        if (value.length < def.minLength) {
            return {
                success: false,
                error: new ValidationError(`${path} must not have fewer than ${def.minLength} elements`),
            };
        }
    }
    // items
...
export function object(lexicons, path, def, value) {
    // type
    if (!isObj(value)) {
        return {
            success: false,
...
export function validateOneOf(lexicons, path, def, value, mustBeObj = false) {
    let concreteDef;
    if (def.type === 'union') {
        ...
        if (!refsContainType(def.refs, value.$type)) {
            if (def.closed) {
                return {
                    success: false,
                    error: new ValidationError(`${path} $type must be one of ${def.refs.join(', ')}`),
                };
            }
            return { success: true, value };
        }
