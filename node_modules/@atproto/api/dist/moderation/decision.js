...
import { BLOCK_BEHAVIOR, CUSTOM_LABEL_VALUE_RE, HIDE_BEHAVIOR, MUTEWORD_BEHAVIOR, MUTE_BEHAVIOR, NOOP_BEHAVIOR, } from './types.js';
...
export class ModerationDecision {
    ...
    addLabel(target, label, opts) {
        ...
        const labelDef = CUSTOM_LABEL_VALUE_RE.test(label.val)
            ? opts.labelDefs?.[label.src]?.find((def) => def.identifier === label.val) || LABELS[label.val]
            : LABELS[label.val];
        if (!labelDef) {
