 ...
 */
export function matchMuteWords({ mutedWords, text, facets, outlineTags, languages, actor, }) {
    const exception = LANGUAGE_EXCEPTIONS.includes(languages?.[0] || '');
    const tags = []
        .concat(outlineTags || [])
        .concat((facets || []).flatMap((facet) => facet.features.filter(AppBskyRichtextFacet.isTag).map((tag) => tag.tag)))
        .map((t) => t.toLowerCase());
    const matches = [];
    outer: for (const muteWord of mutedWords) {
        ...
        if (muteWord.expiresAt && muteWord.expiresAt < new Date().toISOString())
            continue;
        if (muteWord.actorTarget === 'exclude-following' &&
